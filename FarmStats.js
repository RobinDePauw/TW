...
/*==== scraper functions ====*/	
	 ...
	 */
	twcheese.requestDocumentBody = function(targetUrl)
	{
...
/*==== enhancement functions ====*/
	 ...
	 */
	twcheese.includeHaulInfo = function(gameDoc)
	{		
		...
		for(var i=1; i < commandsTable.rows.length - selectorRow; i++)
		{
			...
			if(commandsTable.rows[i].cells[0].getElementsByTagName('img')[0].src.search('return') !=-1)
...
/*==== main ====*/
	...
	twcheese.loadHaulInfo = function()
	{
		...
		setTimeout(
			function()
			{
				twcheese.includeHaulInfo((window.frames.length>0)?window.main.document:document);
